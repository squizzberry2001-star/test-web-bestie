...
var DEFAULT_ADMIN_CONFIG = {
  ...
  masters: {
    auditors: AUDITOR_MASTER_LIST.slice(),
    stores: [],
...
function saveReport_(payload) {
  ...
  var store = String(payload.toko || payload.store || '').trim();
  ...
  var matchedAuditor = AUDITOR_MASTER_LIST.filter(function(name) {
    return normalizeText_(name) === normalizeText_(auditorInput);
  })[0];
  var auditorFolderName = matchedAuditor || auditorInput || 'Auditor Lainnya';
  var reportId = String(payload.reportId || buildReportId_(visitDate, auditorFolderName, store));
  var fileName = String(payload.fileName || ('VisitReport_' + sanitizeFileName_(store) + '_' + sanitizeFileName_(auditorFolderName) + '_' + visitDate + '.pdf'));
...
function mergeAdminConfig_(rawConfig) {
  ...
  if (raw.masters) {
    if (Array.isArray(raw.masters.auditors) && raw.masters.auditors.length) {
      merged.masters.auditors = uniqueNormalizedList_(raw.masters.auditors);
    }
    if (Array.isArray(raw.masters.stores) && raw.masters.stores.length) {
      merged.masters.stores = uniqueNormalizedList_(raw.masters.stores);
    }
