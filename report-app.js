// ==========================================================
    ...
    function getFormData() {
        ...
        const auditorMeta = window.RBReportAdminEnhancer && typeof window.RBReportAdminEnhancer.getSelectedAuditorMeta === 'function'
            ? window.RBReportAdminEnhancer.getSelectedAuditorMeta()
            : { name: getNamaAuditor().trim() || '-', nik: '', label: getNamaAuditor().trim() || '-' };
        const storeMeta = window.RBReportAdminEnhancer && typeof window.RBReportAdminEnhancer.getSelectedStoreMeta === 'function'
            ? window.RBReportAdminEnhancer.getSelectedStoreMeta()
            : { code: '', name: document.getElementById('store').value.trim() || '-', label: document.getElementById('store').value.trim() || '-' };

        return {
            nama: auditorMeta.name || getNamaAuditor().trim() || '-',
            auditorNik: auditorMeta.nik || '',
            namaDropdown: document.getElementById('namaDropdown').value || '',
            namaManual: document.getElementById('namaManual').value.trim() || '',
            store: storeMeta.label || document.getElementById('store').value.trim() || '-',
            storeCode: storeMeta.code || '',
            storeName: storeMeta.name || document.getElementById('store').value.trim() || '-',
    ...
    function buildUploadPayload(data, fileName, pdfBase64) {
        ...
        return {
            auditor: data.nama,
            auditorNik: data.auditorNik || '',
            toko: data.store,
            storeCode: data.storeCode || '',
            storeName: data.storeName || data.store,
            storeLabel: data.store || '',
...
async function downloadPPT() {
    ...
    try {
        ...
        titleSlide.addText(`Kode Toko: ${data.storeCode || '-'}
Auditor: ${auditorName}
NIK: ${data.auditorNik || '-'}
...
// ==========================================================
// [RB-UPGRADE-PATCH-V3]
// Actual bottom progress, hidden admin trigger, photo delete,
// auditor autocomplete suggestions, safer Apps Script form POST
// ==========================================================
Object.assign(SECRET_ACTIVITY, {
    AUTH_CODE: (window.RB_CONFIG && window.RB_CONFIG.USAGE_DASHBOARD_CODE) || SECRET_ACTIVITY.AUTH_CODE || '607090'
});
...
const RB_V3_BASE = {
    ...
    uploadPDF,
    prettifyActivityType,
    fetchRemoteUsageDashboard,
    submitSecretAuth
};

function initializeApp() {
    injectReportEnhancementsV3();
    bindStaticEvents();
    updateSecretPanelState();
    setDefaultVisitDate();
    bootstrapDynamicSections();
    Promise.resolve(hydrateFromLocalDB())
        .catch(error => {
            console.error('Hydrate lokal gagal', error);
        })
        .finally(() => {
            ensureQscPreviewMetadataV3();
            syncAuditorAutocompleteInputV3();
            updateProgressTracker();
        });
    initializeRealtimeUsageTracking();
    checkApiConnection();
...
function injectReportEnhancementsV3() {
    injectAuditorAutocompleteV3();
    injectProgressDockV3();
    ensureQscPreviewMetadataV3();
    enhanceUploadAccessibility();
    hideSecretAccessButtonV3();
    installSecretTapTargetsV3();
    bindDelegatedEnhancementsV3();
    ...
    if (buttonGroup && !document.getElementById('buttonHelperText')) {
        ...
        note.id = 'buttonHelperText';
        note.className = 'section-helper-note';
        note.textContent = 'Tip: progress di bawah selalu mengikuti item form yang aktif. Ketuk status integrasi 5x untuk membuka panel aktivitas.';
        buttonGroup.insertAdjacentElement('afterend', note);
    }
...
function hideSecretAccessButtonV3() {
    const button = document.getElementById('secretAccessBtn');
    if (!button) return;
...
function installSecretTapTargetsV3() {
    if (RB_V3.secretTargetsBound) return;
    RB_V3.secretTargetsBound = true;
    const targets = [
        document.getElementById('systemStatusBar'),
        document.querySelector('.header-content .eyebrow'),
        document.querySelector('.header-content h1')
    ].filter(Boolean);

    targets.forEach(target => {
        if (target.dataset.secretTapBound === '1') return;
        target.dataset.secretTapBound = '1';
        target.addEventListener('click', registerSecretTapV3);
    });
}

function registerSecretTapV3() {
    clearTimeout(RB_V3.secretTapTimer);
    RB_V3.secretTapCount += 1;
    if (RB_V3.secretTapCount >= 5) {
        RB_V3.secretTapCount = 0;
        openSecretPanel();
        return;
    }
    RB_V3.secretTapTimer = setTimeout(() => {
        RB_V3.secretTapCount = 0;
    }, 2200);
}

function bindDelegatedEnhancementsV3() {
    if (RB_V3.delegatedBound) return;
    RB_V3.delegatedBound = true;

    document.addEventListener('click', event => {
        const clearButton = event.target.closest('[data-clear-preview]');
        if (clearButton) {
            event.preventDefault();
            event.stopPropagation();
            clearPhotoPreviewV3(clearButton.dataset.clearPreview, clearButton.dataset.clearInput || '');
        }
    }, true);
}

function injectAuditorAutocompleteV3() {
    const select = document.getElementById('namaDropdown');
    const manual = document.getElementById('namaManual');
    if (!select || document.getElementById('auditorAutocompleteInput')) return;

    const label = document.querySelector('label[for="namaDropdown"]');
    const wrapper = document.createElement('div');
    wrapper.className = 'auditor-autocomplete-wrap';
    wrapper.innerHTML = `
        <input type="text" id="auditorAutocompleteInput" class="auditor-autocomplete-input" list="auditorAutocompleteList" placeholder="Ketik nama auditor..." autocomplete="off" spellcheck="false">
        <datalist id="auditorAutocompleteList"></datalist>
    `;

    select.parentNode.insertBefore(wrapper, select);
    if (label) label.setAttribute('for', 'auditorAutocompleteInput');

    select.classList.add('auditor-hidden-select');
    ...
    populateAuditorAutocompleteV3();

    const input = document.getElementById('auditorAutocompleteInput');
    input.addEventListener('input', () => {
        syncAuditorAutocompleteSelectionV3();
        updateProgressTracker();
    });
    ...
    input.addEventListener('blur', syncAuditorAutocompleteSelectionV3);

    syncAuditorAutocompleteInputV3();
}

function populateAuditorAutocompleteV3() {
    const list = document.getElementById('auditorAutocompleteList');
    const select = document.getElementById('namaDropdown');
...
function syncAuditorAutocompleteSelectionV3() {
    const input = document.getElementById('auditorAutocompleteInput');
    const select = document.getElementById('namaDropdown');
    const manual = document.getElementById('namaManual');
    if (!input || !select || !manual) return;

    const value = String(input.value || '').trim();
    const matchingOption = Array.from(select.options).find(option => {
        if (!option.value || option.value === '__MANUAL__') return false;
        return normalizeText(option.value) === normalizeText(value);
    });
...
function syncAuditorAutocompleteInputV3() {
    const input = document.getElementById('auditorAutocompleteInput');
    const select = document.getElementById('namaDropdown');
    const manual = document.getElementById('namaManual');
    if (!input || !select || !manual) return;
...
function injectProgressDockV3() {
    if (document.getElementById('reportProgressDock')) return;
    const dock = document.createElement('section');
    dock.id = 'reportProgressDock';
    dock.className = 'report-progress-dock';
    dock.setAttribute('aria-live', 'polite');
    dock.innerHTML = `
        <div class="report-progress-topline">
            <div>
                <strong id="reportProgressDockLabel">0% selesai</strong>
                <span id="reportProgressDockMeta">0/0 item terisi</span>
            </div>
            <span class="report-progress-badge" id="reportProgressDockBadge">Draft</span>
        </div>
        <div class="report-progress-track" aria-hidden="true">
            <div class="report-progress-fill" id="reportProgressDockFill"></div>
        </div>
        <div class="report-progress-hint" id="reportProgressDockHint">Mulai isi data auditor, store, dan tanggal.</div>
    `;
    document.body.appendChild(dock);
    document.body.classList.add('has-progress-dock');
}

function buildProgressItemV3(label, done) {
    return { label, done: Boolean(done) };
}

function isFilledV3(value) {
    return Boolean(String(value == null ? '' : value).trim());
}

function collectObservationProgressItemsV3(tableBodyId, label) {
    const rows = Array.from(document.querySelectorAll(`#${tableBodyId} tr`));
    return rows.flatMap((row, index) => {
        const textareas = row.querySelectorAll('textarea');
        const dateInput = row.querySelector('input[type="date"]');
        return [
            buildProgressItemV3(`${label} row ${index + 1} - temuan`, isFilledV3(textareas[0]?.value)),
            buildProgressItemV3(`${label} row ${index + 1} - dampak`, isFilledV3(textareas[1]?.value)),
            buildProgressItemV3(`${label} row ${index + 1} - penyebab`, isFilledV3(textareas[2]?.value)),
            buildProgressItemV3(`${label} row ${index + 1} - tindakan`, isFilledV3(textareas[3]?.value)),
            buildProgressItemV3(`${label} row ${index + 1} - deadline`, isFilledV3(dateInput?.value)),
            buildProgressItemV3(`${label} row ${index + 1} - hasil`, isFilledV3(textareas[4]?.value))
        ];
    });
}

function collectPhotoProgressItemsV3(gridId, label) {
    const cells = Array.from(document.querySelectorAll(`#${gridId} .photo-cell`));
    return cells.flatMap((cell, index) => [
        buildProgressItemV3(`${label} foto ${index + 1}`, Boolean(cell.querySelector('.photo-preview img'))),
        buildProgressItemV3(`${label} deskripsi ${index + 1}`, isFilledV3(cell.querySelector('.photo-description')?.value))
    ]);
}

function collectReportProgressItemsV3() {
    const items = [
        buildProgressItemV3('Nama auditor', isFilledV3(getNamaAuditor())),
        buildProgressItemV3('Store', isFilledV3(document.getElementById('store')?.value)),
        buildProgressItemV3('Tanggal visit', isFilledV3(document.getElementById('tanggal')?.value)),
        buildProgressItemV3('Store Leader', isFilledV3(document.getElementById('storeLeader')?.value)),
        buildProgressItemV3('Level Store Leader', isFilledV3(document.getElementById('storeLeaderLevel')?.value)),
        buildProgressItemV3('Shift Leader', isFilledV3(document.getElementById('shiftLeader')?.value)),
        buildProgressItemV3('Level Shift Leader', isFilledV3(document.getElementById('shiftLeaderLevel')?.value)),
        buildProgressItemV3('Link assignment', isFilledV3(document.getElementById('storeAssignmentLink')?.value))
    ];

    Array.from(document.querySelectorAll('#crewItems .list-item')).forEach((row, index) => {
        const name = row.querySelector('.crew-name')?.value || '';
        const level = row.querySelector('.crew-level-select')?.value || '';
        if (!name.trim() && !level.trim()) return;
        items.push(buildProgressItemV3(`Crew ${index + 1} - nama`, isFilledV3(name)));
        items.push(buildProgressItemV3(`Crew ${index + 1} - level`, isFilledV3(level)));
    });

    if (document.getElementById('toggleQSCResult')?.checked) {
        items.push(buildProgressItemV3('Foto QSC / Famitrack', Boolean(document.querySelector('#qscResultPreview img'))));
        items.push(buildProgressItemV3('Deskripsi QSC / Famitrack', isFilledV3(document.getElementById('qscResultDesc')?.value)));
    }

    if (document.getElementById('toggleOPITable')?.checked) {
        items.push(...collectObservationProgressItemsV3('opiTableBody', 'OPI'));
    }

    if (document.getElementById('toggleQSCTable')?.checked) {
        items.push(...collectObservationProgressItemsV3('qscTableBody', 'QSC'));
    }

    if (document.getElementById('toggleFindingEvidence')?.checked) {
        items.push(...collectPhotoProgressItemsV3('findingEvidenceGrid', 'Finding Evidence'));
    }

    if (document.getElementById('toggleCorrectiveAction')?.checked) {
        items.push(...collectPhotoProgressItemsV3('correctiveActionGrid', 'Corrective Action'));
    }
...
function updateProgressTracker() {
    injectProgressDockV3();
    const labelEl = document.getElementById('reportProgressDockLabel');
    const metaEl = document.getElementById('reportProgressDockMeta');
    const fillEl = document.getElementById('reportProgressDockFill');
    const hintEl = document.getElementById('reportProgressDockHint');
    const badgeEl = document.getElementById('reportProgressDockBadge');
    if (!labelEl || !metaEl || !fillEl || !hintEl || !badgeEl) return;

    const items = collectReportProgressItemsV3();
    const total = items.length;
    const completed = items.filter(item => item.done).length;
    const percent = total ? Math.round((completed / total) * 100) : 0;
    const missing = items.filter(item => !item.done);

    labelEl.textContent = `${percent}% selesai`;
    metaEl.textContent = `${completed}/${total} item terisi`;
    fillEl.style.width = `${percent}%`;

    if (!total) {
        badgeEl.textContent = 'Mulai';
        hintEl.textContent = 'Mulai isi data auditor, store, dan tanggal.';
        return;
    }

    if (!missing.length) {
        badgeEl.textContent = 'Siap export';
        hintEl.textContent = 'Semua item aktif sudah terisi. PDF bisa diunduh tanpa perlu cek ulang ke atas.';
        return;
    }

    badgeEl.textContent = `${missing.length} item belum`;
    const previewMissing = missing.slice(0, 2).map(item => item.label).join(' • ');
    hintEl.textContent = `Masih kurang: ${previewMissing}${missing.length > 2 ? ` +${missing.length - 2} lagi` : ''}`;
}

function ensureQscPreviewMetadataV3() {
    const preview = document.getElementById('qscResultPreview');
    if (!preview) return;
    preview.dataset.emptyText = 'Klik untuk upload foto QSC / Famitrack';
    preview.dataset.inputId = 'qscResultPhoto';
}

function buildPreviewMarkupV3({ image = '', previewId = '', inputId = '', emptyText = 'Klik untuk upload foto', alt = 'Uploaded photo' } = {}) {
    if (!image) {
        return `<span class="upload-icon">📷</span><span class="upload-text">${escapeHtml(emptyText)}</span>`;
    }
    return `
        <img src="${escapeHtml(image)}" alt="${escapeHtml(alt)}">
        <button type="button" class="photo-clear-btn" data-clear-preview="${escapeHtml(previewId)}" data-clear-input="${escapeHtml(inputId)}" aria-label="Hapus foto">×</button>
    `;
}

function setPreviewMarkupV3(previewId, inputId, image = '', emptyText = '') {
    const preview = typeof previewId === 'string' ? document.getElementById(previewId) : previewId;
    if (!preview) return;
    const fallbackEmptyText = emptyText || preview.dataset.emptyText || 'Klik untuk upload foto';
    const resolvedInputId = inputId || preview.dataset.inputId || '';
    preview.innerHTML = buildPreviewMarkupV3({
        image,
    ...
    preview.dataset.inputId = resolvedInputId;
    preview.dataset.emptyText = fallbackEmptyText;
}
...
async function handlePhotoUpload(input, previewId) {
    ...
    if (!file) return;
    const dataUrl = await compressImageFile(file);
    setPreviewMarkupV3(previewId, input.id, dataUrl, 'Klik untuk upload foto QSC / Famitrack');
    logActivity('photo_upload', 'Upload foto QSC / Famitrack.');
    scheduleMediaSave();
    updateProgressTracker();
}
...
async function handlePhotoCellUpload(input, previewId) {
    ...
    if (!file) return;
    const dataUrl = await compressImageFile(file);
    setPreviewMarkupV3(previewId, input.id, dataUrl, 'Klik untuk upload foto');
    const gridId = input.closest('.photo-grid')?.id || 'photoGrid';
    const sectionLabel = gridId === 'findingEvidenceGrid'
        ? 'Finding Evidence'
        : (gridId === 'correctiveActionGrid' ? 'Corrective Action' : 'Photo Grid');
    logActivity('photo_upload', `Upload foto pada section ${sectionLabel}.`);
    scheduleMediaSave();
    updateProgressTracker();
}

function clearPhotoPreviewV3(previewId, inputId = '') {
    const preview = document.getElementById(previewId);
    if (!preview) return;
    const resolvedInputId = inputId || preview.dataset.inputId || '';
    const input = resolvedInputId ? document.getElementById(resolvedInputId) : null;
    if (input) input.value = '';
    setPreviewMarkupV3(preview, resolvedInputId, '', preview.dataset.emptyText || 'Klik untuk upload foto');
    logActivity('photo_clear', `Menghapus foto pada ${previewId}.`);
    scheduleMediaSave();
    updateProgressTracker();
}
...
function createPhotoCell(data = {}, uniqueId = '') {
    ...
    const inputId = `photo${uniqueId}`;
    const hasImage = Boolean(data.image);
    return `
        <div class="photo-cell">
            <div class="photo-upload-box" data-photo-trigger="${inputId}">
                <input type="file" id="${inputId}" accept="image/*" hidden>
                <div id="${previewId}" class="photo-preview ${hasImage ? 'has-image' : ''}" data-empty-text="Klik untuk upload foto" data-input-id="${inputId}">
                    ${buildPreviewMarkupV3({ image: data.image || '', previewId, inputId, emptyText: 'Klik untuk upload foto', alt: 'photo' })}
                </div>
            </div>
            <textarea id="desc${uniqueId}" class="photo-description auto-grow" rows="2" placeholder="Deskripsi foto...">${escapeHtml(data.description || '')}</textarea>
        </div>
...
const addCrewItemBaseV3 = RB_V3_BASE.addCrewItem;
function addCrewItem(...args) {
    const result = addCrewItemBaseV3.apply(this, args);
    setTimeout(updateProgressTracker, 0);
    return result;
}

const removeCrewItemBaseV3 = RB_V3_BASE.removeCrewItem;
function removeCrewItem(...args) {
    const result = removeCrewItemBaseV3.apply(this, args);
    setTimeout(updateProgressTracker, 0);
    return result;
}

const addTableRowBaseV3 = RB_V3_BASE.addTableRow;
function addTableRow(...args) {
    const result = addTableRowBaseV3.apply(this, args);
    setTimeout(updateProgressTracker, 0);
    return result;
}

const removeTableRowBaseV3 = RB_V3_BASE.removeTableRow;
function removeTableRow(...args) {
    const result = removeTableRowBaseV3.apply(this, args);
    setTimeout(updateProgressTracker, 0);
    return result;
}

const applySavedStateBaseV3 = RB_V3_BASE.applySavedState;
function applySavedState(saved) {
    applySavedStateBaseV3.call(this, saved);
    ensureQscPreviewMetadataV3();
    setPreviewMarkupV3('qscResultPreview', 'qscResultPhoto', saved.qscResultPhoto?.image || '', 'Klik untuk upload foto QSC / Famitrack');
    syncAuditorAutocompleteInputV3();
    syncAuditorAutocompleteSelectionV3();
    updateProgressTracker();
}
...
async function hydrateFromLocalDB() {
    const result = await RB_V3_BASE.hydrateFromLocalDB.call(this);
    ensureQscPreviewMetadataV3();
    syncAuditorAutocompleteInputV3();
    updateProgressTracker();
    return result;
...
async function confirmClearData() {
    const result = await RB_V3_BASE.confirmClearData.call(this);
    ensureQscPreviewMetadataV3();
    setPreviewMarkupV3('qscResultPreview', 'qscResultPhoto', '', 'Klik untuk upload foto QSC / Famitrack');
    syncAuditorAutocompleteInputV3();
    updateProgressTracker();
    return result;
}
...
async function uploadPDF(payload) {
    ...
    const formBody = new URLSearchParams();
    formBody.append('action', 'upload_report');
    Object.entries(payload || {}).forEach(([key, value]) => {
        formBody.append(key, value == null ? '' : String(value));
    });
    ...
    const status = String(parsed.status || '').trim().toLowerCase();
    if (!response.ok || !['success', 'ok'].includes(status)) {
        throw new Error(parsed.message || `Upload gagal (HTTP ${response.status})`);
    }
    return parsed;
}
...
async function fetchRemoteUsageDashboard(code = getUsageDashboardCode()) {
    ...
    const { response, raw, payload } = await apiGet('usage_dashboard', {
        code,
        activeWindowMs: SECRET_ACTIVITY.ACTIVE_WINDOW_MS
    });
    const status = String(payload.status || '').trim().toLowerCase();
    if (!response.ok || !['success', 'ok'].includes(status)) {
        throw new Error(payload.message || raw || `Dashboard usage gagal (HTTP ${response.status})`);
    }
    return payload;
}
...
async function submitSecretAuth() {
    ...
    try {
        ...
        closeSecretAuthModal({ restoreFocus: false });
        openSecretActivityDrawer();
        logActivity('secret_access', 'Panel aktivitas dibuka dengan kode yang valid.');
        if (errorEl) errorEl.textContent = '';
    } catch (error) {
        clearUsageDashboardSession();
        if (errorEl) errorEl.textContent = error.message || 'Kode dashboard tidak valid.';
...
// ==========================================================
// [RB-UPGRADE-PATCH-V4]
// Corrective overrides to avoid wrapper recursion
// ==========================================================
function addCrewItem(animate = true, data = {}) {
    const crewItems = document.getElementById('crewItems');
    const currentCount = crewItems.querySelectorAll('.list-item').length;
