    ...
    const APP = {
        API_URL: 'https://script.google.com/macros/s/AKfycbybqCTpezmJ-C59oO9u7k3LDae2WZ4I8OnjmYNCXSEwWTn7IZI-gXW7_7QMW5aM9FZn/exec',
    ...
    async function checkApiConnection() {
        ...
        try {
            ...
            if (!response.ok || payload.status !== 'success') {
                throw new Error(payload.message || 'Health check gagal');
            }
    ...
    function bindStaticEvents() {
        ...
        document.querySelectorAll('[data-bullet-target]').forEach(button => {
            button.addEventListener('click', () => insertBulletForTarget(button.dataset.bulletTarget));
        });
    ...
    function bootstrapDynamicSections() {
        ...
        if (!document.querySelector('#findingEvidenceGrid .photo-row')) {
            for (let i = 0; i < APP.DEFAULT_PHOTO_ROWS; i += 1) {
    ...
    function createObservationRow(tableBodyId, data = {}) {
        ...
        tr.innerHTML = `
            <td data-label="Temuan"><textarea class="bullet-textarea" placeholder="Masukkan temuan...">${escapeHtml(data.temuan || '')}</textarea></td>
            <td data-label="Dampak"><textarea class="bullet-textarea" placeholder="Masukkan dampak...">${escapeHtml(data.dampak || '')}</textarea></td>
            <td data-label="Penyebab"><textarea class="bullet-textarea" placeholder="Masukkan penyebab...">${escapeHtml(data.penyebab || '')}</textarea></td>
            <td data-label="Tindakan Perbaikan"><textarea class="bullet-textarea" placeholder="Masukkan tindakan perbaikan...">${escapeHtml(data.tindakan || '')}</textarea></td>
            <td data-label="Tanggal Perbaikan / Deadline"><input type="date" value="${escapeHtml(data.deadline || '')}"></td>
            <td data-label="Hasil"><textarea class="bullet-textarea" placeholder="Masukkan hasil...">${escapeHtml(data.hasil || '')}</textarea></td>
    ...
    async function addCoverPage(doc, data) {
        ...
        doc.roundedRect(16, 88, pageWidth - 32, 84, 8, 8, 'F');
        doc.setDrawColor(217, 226, 236);
        doc.setLineWidth(0.8);
        doc.roundedRect(16, 88, pageWidth - 32, 84, 8, 8, 'S');
        ...
        doc.roundedRect(pageWidth - 104, 102, 70, 42, 8, 8, 'F');
        doc.setTextColor(255, 255, 255);
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(18);
        doc.text('Audit', pageWidth - 69, 123, { align: 'center' });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(11);
        doc.text('Professional Report', pageWidth - 69, 134, { align: 'center' });
        doc.setTextColor(16, 42, 67);
    }
    ...
    function applySavedState(saved) {
        ...
        document.getElementById('toggleQSCResult').checked = saved.showQSCResult ?? true;
        document.getElementById('toggleOPITable').checked = saved.showOPITable ?? true;
        document.getElementById('toggleQSCTable').checked = saved.showQSCTable ?? true;
        document.getElementById('toggleFindingEvidence').checked = saved.showFindingEvidence ?? true;
        document.getElementById('toggleCorrectiveAction').checked = saved.showCorrectiveAction ?? true;
